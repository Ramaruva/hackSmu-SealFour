...
const VideoChat = ({ peerIdToCall, setStartVideoCall }) => {
    ...
    newPeer.on("call", (incomingCall) => {
      navigator.mediaDevices
        ...
        .then((stream) => {
          ...
          incomingCall.answer(stream); // Answer with your stream

          incomingCall.on("stream", (remoteStream) => {
            remoteVideoRef.current.srcObject = remoteStream; // Set remote stream to video element
          });
        });
    });

    return () => {
      if (peer) peer.destroy(); // Clean up when component unmounts
    };
  }, []);

  const startCall = () => {
    if (!peerIdToCall) return; // No peer to call
    ...
    navigator.mediaDevices
      ...
      .then((stream) => {
        ...
        const outgoingCall = peer.call(peerIdToCall, stream); // Initiate call to specific peer
        outgoingCall.on("stream", (remoteStream) => {
          remoteVideoRef.current.srcObject = remoteStream; // Set remote stream
        });

        setCallActive(true); // Call is active
        setCallEnded(false);
      });
  };

  const endCall = () => {
    if (peer) peer.disconnect(); // Disconnect peer
    setCallActive(false); // Update state
    setCallEnded(true);
    setStartVideoCall(false); // Close the video call component
  };
  ...
  return (
    <div className="video-chat-container flex flex-col items-center mt-6">
      ...
      {callEnded && <p className="text-red-600 mt-4">Call Ended</p>}
    </div>
  );
};
